...
# ============================================================================
    ...
    st.caption(
        "Review uploaded images, semantic analysis, verified audio output, "
    ...
    if os.path.exists(uploaded_dir):
        images = [
            f for f in sorted(glob.glob(f"{uploaded_dir}/*"))
    ...
    with metric2:
        st.metric(
            "🎵 Audio Generated",
            "✓ Yes" if os.path.exists(verified_audio) else "Pending"
        )

    with metric3:
        st.metric(
            "📊 Results Data",
            "Available" if os.path.exists(results_file) else "Pending"
        )
    ...
    # =========================================================================
    # LEFT: EXPERIMENT SAMPLES
    # =========================================================================
    with col1:

        st.markdown("### 🖼️ Experiment Samples")

        if images:

            # Select image instead of displaying everything at once
            image_names = [os.path.basename(img) for img in images]

            selected_name = st.selectbox(
                "Select an experiment sample",
                image_names
            )

            selected_index = image_names.index(selected_name)
            img_path = images[selected_index]
            ...
            try:
                ...
                with col_analysis:
                    ...
                    st.write(
                        f"🎬 **Scene:** {analysis['scene']}"
                    )

                    st.write(
                        f"🎨 **Color:** {analysis['dominant_color']}"
                    )

                    st.write(
                        f"🔍 **Objects:** {', '.join(analysis['objects'])}"
                    )

                    st.divider()

                    col_brightness, col_contrast = st.columns(2)
                ...
                # =============================================================
                st.markdown("---")
                st.markdown("### 🎧 Verified Audio Output")
                ...
                if os.path.exists(verified_audio):
                    ...
                    st.success(
                        "Semantic verification completed successfully."
                    )
                    ...
                    with open(verified_audio, "rb") as f:

                        st.download_button(
                            label="📥 Download Verified Audio",
                            data=f.read(),
                            file_name="verified_audio.wav",
                            mime="audio/wav",
                            use_container_width=True
                    ...
                    st.info(
                        "No verified audio is available yet. "
                        "Run the Audio Generation workflow first."
                    )

            except Exception as e:
                st.warning(
                    f"Could not process {img_path}: {str(e)}"
                )
            ...
            st.info(
                ...
                "Go to the Audio Generation page to start an experiment."
            )
    ...
    with col2:
        ...
        st.write(
            f"🎵 **Verified Audio:** "
            f"{'Available ✓' if os.path.exists(verified_audio) else 'Not Available'}"
        )

        st.write(
            f"📊 **Results File:** "
            f"{'Available ✓' if os.path.exists(results_file) else 'Not Available'}"
        )
